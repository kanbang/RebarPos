...
struct DLLIMPEXP CBOQRow
{
	...
	CBOQRow() :
	pos(0), count(0), diameter(0), length1(0), length2(0), isVarLength(Adesk::kFalse), shapeId(AcDbObjectId::kNull)
	{ 
	...
	CBOQRow(const Adesk::Int32 Pos, const Adesk::Int32 Count, const double Diameter, const double Length1, const double Length2, const Adesk::Boolean IsVarLength, const AcDbObjectId ShapeId) :
		pos(Pos), count(Count), diameter(Diameter), length1(Length1), length2(Length2), isVarLength(IsVarLength), shapeId(AcDbObjectId::kNull)
	{ 
		shapeId = ShapeId;
	}
};
